...
"" ""
PENS
"pen-0" 1.0 0 -14835848 false "" "histogram [count link-neighbors] of turtles"
"pen-1" 1.0 1 -5298144 false "" "histogram [count link-neighbors] of turtles"
...
"" ""
PENS
"default" 0.1 2 -16777216 true "" ";;code by Uri Wilensky\nif (not any? turtles) [stop]\nlet max-degree max [count link-neighbors] of turtles\n;; for this plot, the axes are logarithmic, so we can't\n;; use \"histogram-from\"; we have to plot the points\n;; ourselves one at a time\nplot-pen-reset  ;; erase what we plotted before\n;; the way we create the network there is never a zero degree node,\n;; so start plotting at degree one\nlet degree 1\nwhile [degree <= max-degree] [\n  let matches turtles with [count link-neighbors = degree]\n  if any? matches\n    [ plotxy log degree 10\n             log (count matches) 10 ]\n  set degree degree + 1\n]"

@#$#@#$#@
## WHAT IS IT?

You can generate networks using several different models, starting with the Erdös-Renyi (the "randomest") network model, and a few variations. 
...
STATIC-GEO randomly places nodes on a 2-D square area. Each node tries to connect to its NUM-NEIGHBORS closest neighbors. This means that some nodes end up with more neighbors, because they are the closest choice to some other node even though they already have plenty of closer neighbors.

GROWTH starts out with (NUM-NEIGHBORS + 1) nodes forming a fully connected clique. After this, nodes are added one by one and connected to (NUM-NEIGHBORS/2) previously added nodes (with some randomness so that one ends up with approximately the correct average degree for the entire graph when NUM-NEIGHBORS is odd.

RAND-ENCOUNTER places nodes randomly on a 2D square area. They move around randomly until they occupy the same "patch" as another node, at which point they connect. 
